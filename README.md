...
갱신 경로가 두 개다. **원티드와 캐치가 클라우드 IP를 차단하기 때문이다.**

| | 실행 위치 | 주기 | 원티드·캐치 수집 | 저장소에 커밋 |
| --- | --- | --- | --- | --- |
| GitHub Actions | GitHub 서버 | 매일 06:00 KST | 실패 | 안 함 |
| 작업 스케줄러 | 이 PC | 매일 09:00 | 정상 | 함 |

GitHub Actions 에서 두 곳은 실패하지만, 그 경우 **직전 데이터를 그대로 유지**하므로
기존 공고가 사라지지 않는다. 화면 상단에도 "이전 데이터를 표시하고 있습니다" 안내가 뜬다.
점핏·잡코리아·사람인·네이버블로그는 양쪽 모두에서 정상 수집된다.

작업 스케줄러는 `build_jobs.py` 와 `build_hse.py` 를 **둘 다** 돌려 네 개 데이터 파일을 함께 커밋한다.
두 사이트 모두 캐치를 쓰기 때문이다.
...
`scripts/update-jobs.ps1` 이 두 크롤러 수집 → 커밋 → 푸시를 수행하고, 푸시가 일어나면
...
잡코리아가 한글 음차로 적는 대기업 이름은 `COMPANY_ALIASES` 로 영문에 맞춘다
(`에스케이` → `sk`). 표기가 갈리는 회사를 새로 보면 여기에 한 줄 더한다.

**2단계 — 블로그가 다룬 공고.** 블로그 글은 회사명 자리에 글쓴이가 들어가 있어
1단계 방법을 못 쓴다. 대신 **공고의 회사명이 블로그 제목 안에 있으면** 같은 공고로 보고
공고 쪽을 버린다. 회사명이 세 글자 미만이면 엉뚱한 글에 걸리므로 건너뛴다.

블로그 글을 맨 위에 두는 이유는 보건관리자가 직접 정리한 글이라
기업리뷰·근무지 같은 설명이 함께 붙기 때문이다.
대신 **지역·경력 정보가 없어** 그 공고는 경력 필터에서 '경력 미표기' 로 빠진다.

180 → 160건 (공고끼리 10건, 블로그와 겹쳐 10건).

### 블로그 글 선별 기준

'보건관리자 채용' 으로 검색해도 자격증 학원 광고, 안전보건 위탁기관 영업글,
면접학원 후기, 다른 직무 채용글이 함께 걸린다. 제목을 세 겹으로 거른다
(`crawler/hse_naverblog.py` 의 `is_posting`).

1. 직무를 말할 것 — `보건관리자` / `산업간호사` / `health manager`
2. 채용을 말할 것 — `채용` / `공고` / `모집` / `구인` / `영입`
3. 공고가 아닌 글의 표시어가 없을 것 — `응시자격` / `선임자격` / `자격증` / `시험` / `합격` / `면접` / `학원` / `되는법` / `난이도`

56 → 40건. 버려진 16건은 전부 광고·정보글이었다.
