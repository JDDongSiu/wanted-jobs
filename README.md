...
회사명과 제목을 다듬어 비교해 하나만 남긴다 (`crawler/build_hse.py` 의 `drop_duplicates`).

- 회사명은 표기가 갈린다 — `SK㈜ AX` / `SK AX`, `두산로보틱스㈜` / `두산로보틱스`
- 제목은 끝에 붙는 말만 다른 경우가 많다 — `구성원 영입 (여수)` / `구성원 (여수) 채용`

그래서 둘 다 기호·공백과 `채용`·`영입`·`모집`·`구인`·`공고` 를 지우고 비교한다.
남길 쪽은 `SOURCE_PRIORITY` 순서(**잡코리아 → 캐치 → 피플앤잡**)로 고른다.
잡코리아가 경력·고용형태를 가장 잘 채워 주기 때문이다.

블로그 글은 공고가 아니라 공고를 **소개하는 글**이라 대상에서 뺀다.
같은 공고를 다룬 블로그 글이 따로 보이는 것은 의도한 것이다.
